...
export const generateAIReport = async (req, res) => {
    ...
    try {
        ...
        // 8. Fetch generating user info (S17: add who generated the report)
        console.log('🔍 Step 8: Fetching user info...');
        let generatingUser = { name: 'System', email: '', role: '' };
        try {
            if (req.user?.id) {
                const userParams = {
            ...
            console.log(`✅ Report generated by: ${generatingUser.name}`);
        } catch (error) {
            console.warn('⚠️ Could not fetch user info:', error.message);
        }

        // 9. Prepare AI Context with ALL data (S17 + S18: include user info and detailed expenses)
        console.log('🔍 Step 9: Preparing AI context...');
        const aiContext = prepareAIContext(project, milestones, updates, photos, expenses, comments, documents, reportType, generatingUser);

        // 10. Generate Report with Claude AI
        console.log('🤖 Step 10: Calling Claude AI for report generation...');
...
Use clear headers with ---SECTION 1---, ---SECTION 2---, etc. Format tables using pipe characters (|) for columns. Keep professional tone throughout.`,
        ...
        // 11. Clean up AI generated text - IMPROVED CLEANING
        console.log('🔍 Step 11: Cleaning generated text...');
        ...
        // 12. Save Report to DynamoDB
        console.log('🔍 Step 12: Saving report to database...');
        ...
        const reportParams = {
            ...
            Item: {
                ...
                generatedByName: generatingUser.name,
                generatedByRole: generatingUser.role,
 ...
 */
const prepareAIContext = (project, milestones, updates, photos, expenses, comments, documents, reportType, generatingUser = {}) => {
    ...
    // S18: Group expenses by category for detailed liquidation
    ...
    const expenseLiquidation = Object.entries(expensesByCategory).map(([category, items]) => {
        const categoryTotal = items.reduce((sum, item) => sum + (item.amount || 0), 0);
        const itemDetails = items.map(item => `  - ${(item.description || '').trim()}: ₱${(item.amount || 0).toLocaleString()}`).join('\n');
        return `${category} (Subtotal: ₱${categoryTotal.toLocaleString()}):\n${itemDetails}`;
    }).join('\n\n');

    return `You are a professional construction company writing an ACCOMPLISHMENT REPORT FOR EMPLOYEE/TEAM for ${project.name}.

Construction Company: New San Jose Builders Inc.
Report Generated By: ${generatingUser.name || 'System'} (${generatingUser.role || 'Project Manager'})
Generation Date: ${new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
Generation Time: ${new Date().toLocaleTimeString('en-US')}
...
Report Generated By: ${generatingUser.name || 'System'} (${generatingUser.role || 'Project Manager'})
Reporting Period: [Report Period] | Generated: ${new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })} at ${new Date().toLocaleTimeString('en-US')}
...
- Number of Expense Entries: ${validExpenses.length}
- Budget Utilization: ${totalBudget > 0 ? ((totalSpent / totalBudget) * 100).toFixed(1) : 0}%

Expense Liquidation Breakdown by Category:
${expenseLiquidation || 'No expenses recorded for this period'}
...
---FOOTER---
New San Jose Builders Inc. | BuildWise Construction Management
Report generated by ${generatingUser.name || 'System'} on ${new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
