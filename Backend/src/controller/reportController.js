...
export const generateAIReport = async (req, res) => {
    ...
    console.log(`📊 Generating ${reportType} report for project ${projectId} using Claude AI...`);
    ...
    try {
        ...
        try {
            ...
            return res.status(404).json({ message: 'Project not found or table does not exist' });
        }
        ...
        try {
            ...
            console.warn('⚠️ Could not fetch progress updates:', error.message);
        }
        ...
        try {
            ...
            console.warn('⚠️ Could not fetch photos:', error.message);
        }
        ...
        try {
            ...
            console.warn('⚠️ Could not fetch comments:', error.message);
        }
        ...
        try {
            ...
            console.warn('⚠️ Could not fetch documents:', error.message);
        }
        ...
        console.log('🤖 Calling Claude AI for report generation...');
...
MILESTONE STATUS:
${milestones.map(m => `- ${m.name}: ${m.status} (${m.progress || 0}% complete)`).join('\n') || 'No milestones defined'}

BUDGET BREAKDOWN:
${expenses.map(e => `- ${e.description || 'Expense'}: ₱${(e.amount || 0).toLocaleString()}`).slice(0, 10).join('\n') || 'No expenses recorded'}

WRITING STYLE REQUIREMENTS:
- Write in narrative paragraph format like a professional daily report
- Use complete sentences flowing naturally
- Start with today's date (${new Date().toLocaleDateString()}) and weather conditions
