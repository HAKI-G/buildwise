...
function GenerateReportPage() {
    ...
    const handleDownloadPDF = (report) => {
        try {
            ...
            // S17: Add company name
            doc.setFontSize(14);
            doc.setFont('helvetica', 'bold');
            doc.setTextColor(0, 0, 0); // BLACK
            doc.text('New San Jose Builders Inc.', pageWidth / 2, yPosition, { align: 'center' });
            yPosition += 7;
            ...
            // S17: Add report metadata (who generated, when)
            checkAndAddPage(12);
            doc.setFontSize(9);
            doc.setFont('helvetica', 'normal');
            doc.setTextColor(60, 60, 60);
            const generatorName = report.generatedByName || 'Project Manager';
            const generatorRole = report.generatedByRole || '';
            const generationDate = report.createdAt ? new Date(report.createdAt).toLocaleDateString('en-US', { 
                weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' 
            }) : new Date().toLocaleDateString();
            const generationTime = report.createdAt ? new Date(report.createdAt).toLocaleTimeString('en-US') : '';
            
            doc.text(`Generated by: ${generatorName}${generatorRole ? ` (${generatorRole})` : ''}`, margin, yPosition);
            yPosition += 4;
            doc.text(`Date: ${generationDate}${generationTime ? ` at ${generationTime}` : ''}`, margin, yPosition);
            yPosition += 4;
            doc.text(`Company: New San Jose Builders Inc.`, margin, yPosition);
            yPosition += 6;
            ...
            for (let i = 1; i <= pageCount; i++) {
                ...
                doc.text(
                    `Page ${i} of ${pageCount} | New San Jose Builders Inc. | BuildWise Construction Management | ${new Date().toLocaleDateString()}`,
                    pageWidth / 2,
                    pageHeight - 8,
                    { align: 'center' }
                );
