...
function SettingsPage() {
    ...
    const [notifPrefs, setNotifPrefs] = useState({
        inApp_projectUpdates: true,
        inApp_milestoneDeadlines: true,
        inApp_expenseAlerts: true,
        inApp_teamMessages: true,
        inApp_systemAnnouncements: true,
        email_projectUpdates: false,
        email_milestoneDeadlines: true,
        email_expenseAlerts: false,
        email_teamMessages: false,
    ...
    const [avatarFile, setAvatarFile] = useState(null); // Raw file for S3 upload
    ...
    const load2FAStatus = async () => {
        try {
            ...
            const response = await axios.get(`${process.env.REACT_APP_API_URL || 'http://54.251.28.81'}/api/2fa/status`, {
        ...
        try {
            ...
            if (token) {
                ...
                if (profileData.notificationPreferences) {
                    setNotifPrefs(prev => ({ ...prev, ...profileData.notificationPreferences }));
                }
    ...
    const handleAvatarChange = (e) => {
        ...
        if (file) {
            ...
            // Store the raw file for S3 upload
            setAvatarFile(file);
    ...
    const handleSaveProfile = async (e) => {
        ...
        try {
            ...
            // If user selected a new avatar file, upload to S3 first
            if (avatarFile) {
                try {
    ...
    const handleSavePassword = async (e) => {
        ...
        try {
            ...
            const response = await axios.put(
                `${process.env.REACT_APP_API_URL || 'http://54.251.28.81'}/api/users/change-password`,
                {
    ...
    const handleDisable2FA = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(
                `${process.env.REACT_APP_API_URL || 'http://54.251.28.81'}/api/2fa/disable`,
                {
    ...
    const handleRegenerateBackupCodes = async () => {
        try {
            ...
            const response = await axios.post(
                `${process.env.REACT_APP_API_URL || 'http://54.251.28.81'}/api/2fa/regenerate-backup-codes`,
                {},
                {
                    headers: { Authorization: `Bearer ${token}` }
                }
    ...
    const handleSaveNotifPrefs = async () => {
        ...
        try {
            ...
            await userService.updateUserProfile({ notificationPreferences: notifPrefs });
                        ...
                        <form onSubmit={handleSaveProfile}>
                            ...
                            <div className="mb-4">
                                ...
                                <input
                                    ...
                                    onChange={handleProfileChange}
                                    placeholder="e.g. BuildWise Construction Inc."
                                    className="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                                />
                            </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <h2 className="text-2xl font-semibold text-gray-800 dark:text-white">In-App Notifications</h2>
                            </div>
                            ...
                            <div className="space-y-4">
                                {[
                                    { key: 'inApp_projectUpdates', label: 'Project Updates', desc: 'Progress updates, status changes, new comments' },
                                    { key: 'inApp_milestoneDeadlines', label: 'Milestone & Deadlines', desc: 'Upcoming deadlines, milestone completions' },
                                    { key: 'inApp_expenseAlerts', label: 'Expense Alerts', desc: 'Budget warnings, expense approvals, overruns' },
                                    { key: 'inApp_teamMessages', label: 'Team Activity', desc: 'New team members, role changes, assignments' },
                                    { key: 'inApp_systemAnnouncements', label: 'System Announcements', desc: 'Maintenance notices, feature updates' }
                                ].map(item => (
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                            ...
                            <p className="text-sm text-gray-500 dark:text-gray-400 mb-5">
                                Choose which notifications also get sent to your email
                            </p>
                            <div className="space-y-4">
                                {[
                                    { key: 'email_projectUpdates', label: 'Project Updates', desc: 'Get emailed when your projects have major changes' },
                                    { key: 'email_milestoneDeadlines', label: 'Milestone & Deadlines', desc: 'Email reminders for upcoming deadlines' },
                                    { key: 'email_expenseAlerts', label: 'Expense Alerts', desc: 'Budget warning emails for immediate attention' },
                                    { key: 'email_teamMessages', label: 'Team Activity', desc: 'Email when new members join or roles change' },
                                    { key: 'email_systemAnnouncements', label: 'System Announcements', desc: 'Important system-wide emails' }
                                ].map(item => (
                                    <div key={item.key} className="flex items-center justify-between py-2 border-b border-gray-100 dark:border-gray-700 last:border-0">
                                        <div>
                                            <p className="font-medium text-gray-800 dark:text-white">{item.label}</p>
