...
function ProjectsPage() {
    ...
    const handleDelete = async (projectId) => {
        notify.confirm('Are you sure you want to delete this project?', async () => {
            const token = getToken();
            if (!token) {
                navigate('/login');
                return;
            ...
            try {
                ...
                if (err.response?.status !== 404) {
                    notify.error('Failed to delete: ' + (err.response?.data?.message || 'Unknown error'));
                    return;
                }
    ...
    const handleCreateProject = async (e) => {
        ...
        if (!projectName.trim()) validationErrors.push('Project name is required');
        if (!location.trim()) validationErrors.push('Location is required');
        if (!contractor.trim()) validationErrors.push('Contractor name is required');
        if (!dateStarted) validationErrors.push('Date started is required');
        if (!contractCompletionDate) validationErrors.push('Contract completion date is required');
        if (!contractCost || parseFloat(contractCost) <= 0) validationErrors.push('Contract cost must be a positive number');
        if (!projectManager.trim()) validationErrors.push('Project manager name is required');
        
        // Validate project manager name format (at least first and last name)
        ...
        if (contractor.trim() && contractor.trim().length < 3) {
            validationErrors.push('Contractor name must be at least 3 characters');
        }
        ...
        try {
            const response = await axios.post(`${process.env.REACT_APP_API_URL || 'http://54.251.28.81'}/api/projects`, formData, config);
            ...
            <div className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm transition-colors">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-3 sm:mb-4">
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
                        ...
                        {/* Add New Project Button - hidden for VP */}
                        {!isVP && (
                        <button
...
{showModal && (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-gray-200 dark:border-slate-700">
            {/* Modal Header */}
            <div className="sticky top-0 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-slate-900 dark:to-slate-800 border-b border-gray-200 dark:border-slate-700 px-6 py-5 flex items-center justify-between z-10">
