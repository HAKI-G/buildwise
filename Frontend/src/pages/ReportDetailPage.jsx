...
function GenerateReportPage() {
    ...
    const renderSectionContent = (body) => {
        if (body.includes('|')) {
            ...
            if (lines.some(line => line.trim().startsWith('-') || line.trim().startsWith('•'))) {
                ...
                return <p className="text-gray-700 dark:text-slate-300 leading-relaxed">{body}</p>;
            }
        }
    };
    ...
    // PDF Download - Accomplishment Report format
    const handleDownloadPDF = (report) => {
        try {
            ...
            // 1. HEADER with golden borders
            checkAndAddPage(15);
            doc.setLineWidth(2);
            doc.setDrawColor(214, 159, 0); // Golden color
            doc.line(margin, yPosition, pageWidth - margin, yPosition);
            yPosition += 4;

            doc.setFontSize(20);
            doc.setFont('helvetica', 'bold');
            doc.setTextColor(139, 69, 19); // Dark golden
            ...
            if (sections.EMPLOYEE_INFO) {
                ...
                infoLines.forEach((line, idx) => {
                    ...
                    doc.text(label.trim() + ':', margin + (infoOnCurrentRow * contentWidth / itemsPerRow), yPosition);

                    doc.setFont('helvetica', 'normal');
                    doc.setTextColor(50, 50, 50);
                    doc.text(value, margin + (infoOnCurrentRow * contentWidth / itemsPerRow) + 40, yPosition);
            ...
            if (sections.INTRODUCTION) {
                ...
                doc.setFont('helvetica', 'normal');
                doc.setTextColor(50, 50, 50);
                const introLines = doc.splitTextToSize(sections.INTRODUCTION.trim(), contentWidth);
